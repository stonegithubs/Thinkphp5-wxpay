...
* 3. 服务端: 调用\wxpay\WxAppPay::getOpenid($code) 获取openid
* 4. 服务端: 存储openid, 并设置过期时间(为了安全起见, 不可永久有效)
* -----第二阶段 进行支付获取预订单信息-----
* 5. 小程序: 调用wx.request(), 向服务器发送请求
* 6. 服务端: 通过用户身份, 找到对应用户的, 从缓存/数据中获取openid
* 7. 服务端: 调用\wxpay\WxAppPay::prePay($params) 获取预订单信息 prepay_id
* 8. 服务端: 调用\wxpay\WxAppPay::getPayData($prepay_id) 获取小程序支付参数, 返回给小程序
* -----第三阶段 小程序拉起支付 -------
...
*/
class WxAppPay extends WxBase
{
    // APPID：微信分配的小程序ID(如果仅有小程序支付, 可将其放入父类中)
    const APPID       = '';
    // APPSECRET: 小程序密钥
    const APPSECRET   =  '';
    // REQUEST_URL: 下单请求地址
